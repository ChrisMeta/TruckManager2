  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        {CAT_TABS.map(tabKey => <button key={tabKey} className={`tab ${cat===tabKey?'tab-active':''}`} onClick={()=>setCat(tabKey)}>{t(tabKey)}</button>)}
      </div>
        ...
        {items.map(i => {
          ...
          return (
            <div key={`${i.brand}-${i.model}`} className="border border-white/10 rounded-xl p-3 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button className="btn btn-primary" onClick={()=>buy(i,'cash')} disabled={loading}>
                  {loading ? t('buying') : t('buyWithCash')}
                </button>
                <button className="btn" onClick={()=>buy(i,'premium')} disabled={loading}>{t('buy')} ({gems}★)</button>
              </div>
            </div>
          );
        })}
